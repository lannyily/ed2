...
// Bellman-Ford com logaritmo negativo
int bellmanFord(Grafo* grafo, int origem, double* dist, int* pred) {
    int V = grafo->V;
    int E = grafo->E;

    // Verifica confiabilidades inválidas
    for (int j = 0; j < E; j++) {
        double r = grafo->arestas[j].confiabilidade;
        if (r <= 0.0 || r > 1.0) {
            printf("Erro: confiabilidade inválida na aresta (%d -> %d): %.3f\n",
                   grafo->arestas[j].u, grafo->arestas[j].v, r);
            return 0;
        }
    }
    ...
    for (int i = 1; i <= V - 1; i++) {
        for (int j = 0; j < E; j++) {
            ...
            double peso = -log(grafo->arestas[j].confiabilidade); // Peso transformado
        ...
        if (dist[u] != INFINITO && dist[u] + peso < dist[v] - 1e-9)
 {
    ...
    // Verifica ciclos negativos (não deveria ocorrer neste problema)
    for (int j = 0; j < E; j++) {
        int u = grafo->arestas[j].u;
        int v = grafo->arestas[j].v;
        double peso = -log(grafo->arestas[j].confiabilidade);
...
int main() {
    ...
    // Exemplo de arestas com confiabilidade
    ...
    // Alocação dinâmica para evitar erro de array com tamanho variável
